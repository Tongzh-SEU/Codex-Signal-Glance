...
final class TouchBarController: NSObject, NSTouchBarDelegate {
    private static let itemIdentifier = NSTouchBarItem.Identifier("com.wendy.codex-signal-glance.quota")
    private static let trayIdentifier = "com.wendy.codex-signal-glance"
    ...
    func show(snapshot: QuotaSnapshot?, activity: CodexActivitySnapshot) {
        ...
        quotaView.render(snapshot: snapshot ?? lastSnapshot, activity: activity, language: language)
    ...
    func showAgain() {
        isUserHidden = false
        quotaView.render(snapshot: lastSnapshot, activity: lastActivity, language: language)
        isPresented = false
        present()
    }
...
private final class TouchBarQuotaView: NSView {
    private let statusView = TrafficLightStatusView()
    private let fiveHourRow = TouchBarQuotaRow()
    private let sevenDayRow = TouchBarQuotaRow()

    override init(frame frameRect: NSRect) {
        super.init(frame: NSRect(x: 0, y: 0, width: 720, height: 30))
        setupViews()
        render(snapshot: nil, activity: .idle, language: .english)
    }
    ...
    func render(snapshot: QuotaSnapshot?, activity: CodexActivitySnapshot, language: WidgetLanguage) {
        statusView.render(activity: activity)
    ...
    private func setupViews() {
        ...
        let stack = NSStackView(views: [titleLabel, statusView, rows])
        ...
        NSLayoutConstraint.activate([
            ...
            statusView.widthAnchor.constraint(equalToConstant: 62),
